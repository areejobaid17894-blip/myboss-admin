...
export function UnregisteredPage() {
  const { t } = useI18n();
  const { loading, error, squads, unregistered, reload } = useAdminData();
  ...
  const nudgeAll = () => {
    logAction(`Sent registration reminder to ${unregistered.length} unregistered employees.`);
  ...
  return (
    <>
      ...
      <div className="ac-ai-note">✦ <span>{t('unregAiNote')}</span></div>
